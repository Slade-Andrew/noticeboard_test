...
class Model_User extends \Orm\Model
{
	...
	// This function retrives all information required for each notice that contains a comment
	// and is only executed when a notice is clicked on
	public static function get_user_of_comment($id)
	{
		$users = DB::query('SELECT users.id as userid, users.username, comments.id as commentid ,comment 
							FROM users join comments on users.id = comments.user_id 
							WHERE comments.notice_id ='. $id)->execute()->as_array();
